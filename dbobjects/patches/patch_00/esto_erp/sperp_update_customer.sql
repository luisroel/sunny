DROP PROCEDURE IF EXISTS `sperp_update_customer`;
DELIMITER $$
CREATE PROCEDURE `sperp_update_customer`(
	/* Customer */
...
BEGIN
	...
	IF EXISTS(SELECT * FROM tberp_customer CT WHERE CT.Code = Code_p AND CT.CustomerId <> CustomerId_p) THEN
		SET	Msg_p = 'Customer code already exist. Please verify!';
	ELSEIF EXISTS(SELECT * FROM tberp_customer CT WHERE CT.Name = Name_p AND CT.CustomerId <> CustomerId_p) THEN
